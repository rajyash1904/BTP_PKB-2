...
class EntropyBottleneck(tf.keras.layers.Layer):
  ...
  """
    ...
    """
        ...
        likelihood = tf.reshape(
            likelihood, shape
        )  # shape=[channels, batch, length, width, height]
        likelihood = tf.transpose(
            likelihood, order
        )  # shape=[batch size, length, width, height, channels]
    ...
    """
        ...
        a = tf.reshape(
            tf.range(min_v, max_v + 1), [1, 1, max_v - min_v + 1]
        )  # [1, 1, N]
    ...
    """
        ...
        with tf.name_scope(self._name_scope()):
            ...
            code_shape = (
                tf.constant(code_length // channels, dtype="int32"),
                tf.constant(channels, dtype="int32"),
            )  # shape=[-1, channel]
