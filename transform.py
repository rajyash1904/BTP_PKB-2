...
def compress_factorized(cubes, model, ckpt_dir):
    ...
    checkpoint = tf.train.Checkpoint(
        analysis_transform=analysis_transform, estimator=entropy_bottleneck
    )
    ...
    start = time.time()
    ys = tf.map_fn(
        loop_analysis, x, dtype=tf.float32, parallel_iterations=1, back_prop=False
    )
...
def decompress_factorized(strings, min_v, max_v, shape, model, ckpt_dir):
    ...
    entropy_bottleneck = EntropyBottleneck()
    checkpoint = tf.train.Checkpoint(
        synthesis_transform=synthesis_transform, estimator=entropy_bottleneck
    )
    status = checkpoint.restore(tf.train.latest_checkpoint(ckpt_dir))
    ...
    start = time.time()
    xs = tf.map_fn(
        loop_synthesis, ys, dtype=tf.float32, parallel_iterations=1, back_prop=False
    )
...
def compress_hyper(cubes, model, ckpt_dir, decompress=False):
    ...
    start = time.time()
    zs = tf.map_fn(
        loop_hyper_encoder, ys, dtype=tf.float32, parallel_iterations=1, back_prop=False
    )
    ...
    args = (ys, locs, scales)
    y_strings, y_min_vs, y_max_vs = tf.map_fn(
        loop_range_encode,
        args,
...
def decompress_hyper(
    ...
    start = time.time()
    zs = entropy_bottleneck.decompress(
        z_strings, z_min_v, z_max_v, z_shape, z_shape[-1]
    )
